...
#wgan training with real and fake separate training discriminator
...
#
#     optimizer = optim.Adam(generator.parameters(), lr=opt.generatorLR, betas = (0.5, 0.999))
#     optim_discriminator = optim.Adam(discriminator.parameters(), lr=opt.discriminatorLR, betas = (0.5, 0.999))
...
#     #training
#     gen_iterations = 3
#     for epoch in range(opt.nEpochs-3):
#         print("epoch {}\n".format(epoch))
...
#
#                             discriminator_real_loss = adversarial_criterion(
...
#
#                             # train with fake
#                             discriminator_fake_loss = adversarial_criterion(
...
#wgan training with real+fake combined training discriminator
# def train_multiple(generator, discriminator, opt, dataloader, writer, scale):
#     feature_extractor = FeatureExtractor(torchvision.models.vgg19(pretrained=True))
#
#     content_criterion = nn.MSELoss()
                ...
                while j<14:
                    ...
                    if opt.cuda:
                        ...
                        if (j == 14):
                            sys.stdout.write(
                                '\repoch [%d/%d] content_Loss: %.4f' % (
                                   epoch, opt.nEpochs, total_loss))


            imsave(fake_img.cpu().data, train=True, epoch=count, image_type='new', ang_res=True, row_ind=i, column_ind=j)
            imsave(gt_center_img, train=True, epoch=count, image_type='real_center', ang_res=True)
            imsave(gt_horizontaltop_img, train=True, epoch=count, image_type='real_horizontaltop', ang_res=True)
            imsave(gt_horizontalbottom_img, train=True, epoch=count, image_type='real_horizontalbottom', ang_res=True)
            imsave(gt_verticalleft_img, train=True, epoch=count, image_type='real_verticalleft', ang_res=True)
            imsave(gt_verticalright_img, train=True, epoch=count, image_type='real_verticalright', ang_res=True)
            writer.add_scalar(" per epoch/angres lr", optimizer.param_groups[0]['lr'], epoch + 1)
            count += 1
                # else:
                #     imsave(fake_img.cpu().data, train=False, epoch=epoch, image_type='new', ang_res=True)
